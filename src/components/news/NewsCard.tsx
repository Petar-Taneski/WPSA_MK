...
const NewsCard = ({ article }: NewsCardProps) => {
  ...
  return (
    <article className="h-full">
      <a
        href={getPostUrl()}
        className="block h-full focus:outline-none focus:ring-2 focus:ring-primary/50 text-inherit no-underline"
      ...
      >
        <div className="flex flex-col h-full overflow-hidden transition-shadow duration-300 bg-white shadow-md group hover:shadow-lg">
          <div className="relative flex items-center justify-center h-48 overflow-hidden max-md:h-80 bg-gray-50">
            <img
              src={displayImageUrl}
              alt={title}
